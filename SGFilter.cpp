...
if(!v){
//log_printf("Error: Allocation failure.");
exit(1);
}
return v-nl+1;
}
...
char convlv(double data[],unsigned long n,double respns[],unsigned long m,int isign,double ans[])
{
void realft(double data[],unsigned long n,int isign);
void twofft(double data1[],double data2[],double fft1[],double fft2[],unsigned long n);
unsigned long i,no2;
...
char sgfilter_main(double weightedAvgs[], double smoothedAvgs[], int arraySize, int leftFrame, int rightFrame, int ld, int polyOrder)
{
	char retval=0;
	double *yr= dvector(1,arraySize);
	#if CONVOLVE_WITH_NR_CONVLV
	double *yf= dvector(1,2*arraySize);
	#else
	double *yf= dvector(1,arraySize);
	#endif

	int index = 0;
	while(index < arraySize)
	{
		yr[index+1] = weightedAvgs[index];
		index++;
	}

	if(retval = (sgfilter(yr,yf,arraySize,leftFrame,rightFrame,ld,polyOrder)))
		return retval;
	...
	while(index < arraySize)
	{
		smoothedAvgs[index] = yf[index+1];
		index++;
	}
	

	free_dvector(yr,1,arraySize);
	#if CONVOLVE_WITH_NR_CONVLV
	free_dvector(yf,1,2*arraySize);
	#else
	free_dvector(yf,1,arraySize);
	#endif
	return retval;
}
